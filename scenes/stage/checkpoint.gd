...
func _ready() -> void:
	...
	for enemy : Character in enemies.get_children():
		enemy_data.append(EnemyData.new(enemy.type, enemies.global_position))
		enemy.queue_free()
...
func can_spawn_enemies() -> bool:
	return enemy_data.size() > 0 and active_enemy_counter < nb_sim_enemies

func on_enemy_death(_enemy : Character) -> void:
	active_enemy_counter -= 1
	if active_enemy_counter == 0 and enemy_data.size() == 0:
		StageManager.checkpoint_end.emit()
		queue_free()
