...
func set_heading() -> void:
	if player == null and not can_move():
		return
...
func on_emit_damage(receiver : DamageReceiver) -> void:
	receiver.damage_received.emit(damage, heading, DamageReceiver.HITtype.KNOCKBACK)
	time_last_attack = Time.get_ticks_msec()
	state = State.IDLE
